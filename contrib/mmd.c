...
static mmd_t    *mmd_add(mmd_t *parent, mmd_type_t type, int whitespace, const char *text, char *url);
      ...
      */
      ...
      if ((temp - lineptr) <= 6)
      {
        ...
        */
        ...
        type  = (mmd_type_t)(MMD_TYPE_HEADING_1 + (temp - lineptr - 1));
        block = NULL;
...
mmd_add(mmd_t      *parent,             /* I - Parent node */
        ...
        const char       *text,         /* I - Text, if any */
        char       *url)                /* I - URL, if any */
{
  mmd_t         *temp;                  /* New node */


  if ((temp = (mmd_t *)calloc(1, sizeof(mmd_t))) != NULL)
  {
    if (parent)
    {
     /*
