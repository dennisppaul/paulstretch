...
const float Hilbert::coefl[]={0.6923877778065, 0.9360654322959, 0.9882295226860, 0.9987488452737};
const float Hilbert::coefr[]={0.4021921162426, 0.8561710882420, 0.9722909545651, 0.9952884791278};
...
void BinauralBeats::process(float *smpsl,float *smpsr,int nsmps,float pos_percents){
//	for (int i=0;i<nsmps;i++) smpsl[i]=smpsr[i]=sin(30.0*M_PI*i*2.0/nsmps)*0.4;
	...
	if (pars.free_edit.get_enabled()){
		...
		for (int i=0;i<nsmps;i++){
			float inl=smpsl[i];
			float inr=smpsr[i];
			float outl=inl*(1.0-mono)+inr*mono;
			float outr=inr*(1.0-mono)+inl*mono;

			smpsl[i]=outl;
			smpsr[i]=outr;
		};

		float freq=pars.free_edit.get_value(pos_percents);
