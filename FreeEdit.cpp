...
void FreeEdit::deep_copy_from(const FreeEdit &other){
	...
	if (other.curve.data&&other.curve.size){
		curve.data=new float[curve.size];
...
void FreeEdit::get_curve(int datasize,float *data,bool real_values){
	int npos_used=0;
	...
	float posx[npos_used],posy[npos_used];
	int k=0;
	for (int i=0;i<npos;i++){
		if (is_enabled(i)){
			posx[k]=get_posx(i);
	...
	for (int i=0;i<datasize;i++){
		float x=(float)i/(float)datasize;
		...
		float y=y=posy[p1]*(1.0-x0)+posy[p2]*x0;
		data[i]=y;
	};
	...
	if (smooth>0.01){
		...
		float a=exp(log(0.25)/(smooth*smooth*datasize*0.25));
...
float FreeEdit::get_value(float x){
	if (!curve.data) {
